...
import TodoListtItem from "./TodoListItem";
...
const TodoList = ({
  ...
  const handleCopy = () => {
    const shareLink = `${"todoList공유할링크"}/shard/${ownerUserId}`;
    copy(shareLink)
      .then(() => {
    ...
    <section className="min-h-[70vh] bg-[#69CFCF]">
      <div className="w-full max-w-[800px] p-[20px] mx-auto">
        ...
        <article className="flex flex-col sm:flex-row gap-4 mt-8">
          {/* w가 640px 이하로 떨어질 경우 요소들이 flex-row에서 flex-col로 정렬되도록 함 */}
        ...
        {todoListData?.length >= 1 ? (
          <ul className="flex flex-col gap-6">
            {(todoListData ?? []).map((todo) => {
              return (
                <TodoListtItem key={todo?.id} todo={todo} onDelete={() => {}} />
              );
            })}
          </ul>
        ) : (
